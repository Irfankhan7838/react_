...
if ($data['action'] == 'updt_patient_detail') {
    //UPDATING PATIENT SINGLE COLUMN DATA
	$err= validation($data);		
	if(!empty($err)){
		echo json_encode(array('status'=>'fail','code'=>'0','msg'=>$err));
		die();
	 
	}else if(empty($data['updpatient_id'])){
		...
		// $patient_count = mysqli_num_rows($existing_patient_query);
		// if ($patient_count >0) {
		// 	echo json_encode(array('status'=>'fail','code'=>'0','msg'=>'patient with this relation already exist'));
		// 	die();
		// }

		$sql = "update Patient set  P_User_id = '".$_SESSION['user_id']."' ,  P_Name = '".$data['UdtName']."' , P_Email = '".$data['Udtemail']."' , P_Phone = '".$data['Udtphone_number']."' , P_DOB = '".$data['Udtdob']."' , P_Address = '".$data['UdtAddress']."' , P_Comment = '".$data['UdtComment']."' , P_relation = '".$data['Udtrelation']."', P_patient_source = '".$data['Udtpatient_source']."' where P_id ='".$data['updpatient_id']."'";
...
if ($data['action'] == 'get_patient') {
	...
	if(empty($data['patient_id'])){
		...
		$sql = "select P_Name,P_Phone,P_Email,P_Address,P_DOB,P_relation,P_patient_source,P_Comment from Patient where P_id ='".$data['patient_id']."'";
	          			...
	          			<div class="col-sm-12 table-responsive">
							<table class="table table_max_width">
								<tr>
									...
									<td >Name: <span class=" mr-1" id="span_P_Name">'.$patient_data['P_Name'].'</span></td>

																			
									<td >Email: <span class=" mr-1" id="span_P_Email">'.$patient_data['P_Email'].'</span></td>

																			
									<td >Phone: <span class=" mr-1" id="span_P_Phone">'.$patient_data['P_Phone'].'</span></td>
								</tr>
					
							
								<tr  >
																			
									<td>DOB: <span class=" mr-1" id="span_P_DOB">'.$patient_data['P_DOB'].'</span></td>

									<td >Relation: <span class=" mr-1" id="span_P_relation">'.$patient_data['P_relation'].'</span></td>

									<td>Patient source:<span class=" mr-1" id="span_P_patient_source">'.$patient_data['P_patient_source'].'</span></td>
								</tr>
			
								<tr >
																				
									<td >Address: <span class=" mr-1" id="span_P_Address">'.$patient_data['P_Address'].'</span></td>

									<td >Comment: <span class=" mr-1" id="span_P_Comment">'.$patient_data['P_Comment'].'</span></td>
								</tr>

								<tr> <td width="100% text-center"><button onclick="open_update_patient_modal()" class="btn btn-primary">Edit <i class="fa fa-pencil" ></i></button></td></tr>
							</table>
						</div>
	          		</div>';

			echo json_encode(array('status'=>'success','code'=>'1','data'=>$data, 'total_data' => $total, 'tsql' => $total_due_sql));
		}
...
if ($data['action'] == 'get_patient_treatment') {
	...
	if(empty($data['patient_id'])){
		...
		die();
			...
			$sql = "select T_id,T_patient_id,T_cost,T_treatment_name,T_appointment_datetime from Treatment where T_patient_id ='".$data['patient_id']."' and T_status ='0' order by T_appointment_datetime desc LIMIT $offset, $records_per_page";
			$query = sqlQuery($sql,"select"); 
			...
			$data .= '
			    ...
			    while ($treatment_data = mysqli_fetch_assoc($query)) {
			             	$data .= " <tr>
					               ...
					               <td>".date('j M Y H:i a', strtotime($treatment_data['T_appointment_datetime']))."</td>
...
if ($data['action'] == 'get_patient_payment') {
	...
	if(empty($data['patient_id'])){
	    ...
	    $sql = "select Pay_id,P_id ,Pay_method,Pay_datetime,Pay_patient_id,Pay_payment from Patient RIGHT JOIN Payment ON Patient.P_id = Payment.Pay_patient_id where P_id ='".$data['patient_id']."' and Pay_status ='0' order by Pay_datetime desc LIMIT $offset, $records_per_page";
		$query = sqlQuery($sql,"select"); 
			...
			$data .= '
			    ...
			    while ($payment_data = mysqli_fetch_assoc($query)) {
			             	$data .= " <tr>
				               ...
				               <td>".date('j M Y H:i a', strtotime($payment_data['Pay_datetime']))."</td>
...
if ($data['action'] == 'add_treatment') {
	...
	if(empty($data['patient_id'])){
		...
		if (!empty($data['treatment_id'])) {
			 $sql = "update Treatment set T_treatment_name = '".$data['treatment_name']."',T_appointment_datetime = '".$data['appointment_date']."' ,T_cost= '".$data['treatment_cost']."' where T_id = '".$data['treatment_id']."'";
			$update = sqlQuery($sql,"update"); 
			...
			$sql = "insert into Treatment (T_patient_id,T_userid,T_treatment_name,T_appointment_datetime,T_cost) values('".$data['patient_id']."','".$_SESSION['user_id']."','".$data['treatment_name']."','".$data['appointment_date']."','".$data['treatment_cost']."')";
			$insert = sqlQuery($sql,"insert"); 
...
if ($data['action'] == 'add_payment') {
	...
	if(empty($data['payment_patient_id'])){
		...
		if (!empty($data['payment_id'])) {
			 $sql = "update Payment set Pay_userid = '".$_SESSION['user_id']."',Pay_payment = '".$data['payment']."' ,Pay_datetime= '".$data['Payment_datetime']."' ,Pay_method = '".$data['Payment_method']."' where Pay_id = '".$data['payment_id']."'";
			$update = sqlQuery($sql,"update"); 
			...
			$sql = "insert into Payment (Pay_userid,Pay_patient_id,Pay_payment,Pay_method,Pay_datetime) values('".$_SESSION['user_id']."','".$data['payment_patient_id']."','".$data['payment']."','".$data['Payment_method']."','".$data['Payment_datetime']."')";
			$insert = sqlQuery($sql,"insert"); 
